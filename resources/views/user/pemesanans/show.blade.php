...
@section('content')
    <div class="max-w-5xl mx-auto mt-10">
        ...
        <div class="bg-gray-900 rounded-2xl shadow-xl overflow-hidden flex">
            ...
            <!-- POSTER -->
            <div class="w-1/3">
                <img src="{{ asset('posters/' . $pemesanan->jadwal->film->poster) }}" class="h-full w-full object-cover">
            </div>
            ...
            <div class="w-2/3 p-6 text-gray-100 flex flex-col justify-between">
                        ...
                        @php
                            $info = [
                                ...
                                [
                                    ...
                                    'M3 21h18M9 8h6m-6 4h6m-6 4h6M5 21V5a2 2 0 012-2h10a2 2 0 012 2v16',
                                ],
                                [
                                    'SEAT',
                                    $pemesanan->kursi->pluck('nomor_kursi')->implode(', '),
                                    'M6 18v-5a3 3 0 013-3h6a3 3 0 013 3v5M5 21h14',
                                ],
                            ];
                        @endphp
                ...
                @php
                    $qrData = [
                        'booking_id' => 'TX-' . str_pad($pemesanan->id, 6, '0', STR_PAD_LEFT),
                        'film' => $pemesanan->jadwal->film->judul,
                        'tanggal' => $pemesanan->jadwal->tanggal,
                        'jam' => $pemesanan->jadwal->jam,
                        'studio' => $pemesanan->jadwal->studio->nama,
                        'kursi' => $pemesanan->kursi->pluck('nomor_kursi')->implode(', '),
                ...
                <div class="mt-6 bg-gray-800 rounded-xl p-4 flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="bg-white p-2 rounded-lg shadow">
                            {!! QrCode::size(120)->generate($qrJson) !!}
                        </div>
